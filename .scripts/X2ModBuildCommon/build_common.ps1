...
class BuildProject {
	...
	BuildProject(
		...
		$this.finalModPath = $modDestinationPath
	}
	...
	[void]_ConfirmPaths() {
		...
		{
			ThrowFailure ("The path '{0}' doesn't exist. Please adjust the xcom.highlander.sdkroot variable in your user config and retry." -f $this.sdkPath)
		}
		elseif (!(Test-Path $this.gamePath))
		{
			ThrowFailure ("The path '{0}' doesn't exist. Please adjust the xcom.highlander.gameroot variable in your user config and retry." -f $this.gamePath)
		}
	}
	...
	[void]_SetupUtils() {
		...
		$this.finalModPath = "$($this.finalModPath)\$($this.modNameCanonical)"
	...
	[void]_RunMakeMod() {
		# Delete the .u file if it exists to ensure a clean compilation
		$packagePath = "$($this.sdkPath)\XComGame\Script\$($this.modNameCanonical).u"
		if (Test-Path $packagePath) {
			Write-Host "Deleting existing $($this.modNameCanonical).u file to ensure clean compilation..."
			Remove-Item $packagePath -Force
			Write-Host "Adding a delay after file deletion..."
		...
		# Add a longer delay after compilation to ensure file operations complete
		Write-Host "Adding a longer delay after compilation to ensure file operations complete..."
		Start-Sleep -Seconds 10

		# Verify that the .u file exists and is accessible
		if (Test-Path $packagePath) {
			Write-Host "$($this.modNameCanonical).u file exists and is accessible."
		} else {
			Write-Host "Warning: $($this.modNameCanonical).u file does not exist after compilation!"
		}
	}
	...
	[void]_CopyScriptPackages() {
		# Move packages to staging (optimized to avoid duplication)
		foreach ($name in $this.modScriptPackages) {
			if ($this.cookHL -and $global:nativescriptpackages.Contains($name))
			{
				# This is a native (cooked) script package -- copy important upks
				# We still use Copy-Item for these as they might be needed by other mods
			...
			{
				# Or this is a non-native package - move instead of copy to avoid duplication
				$sourcePath = "$($this.sdkPath)\XComGame\Script\$name.u"
				$destPath = "$($this.stagingPath)\Script\$name.u"

				# Create a backup before moving
				$backupPath = "$($this.sdkPath)\XComGame\Script\$name.u.bak"

				try {
					# Create backup first
					if (Test-Path $sourcePath) {
						Copy-Item $sourcePath $backupPath -Force -ErrorAction Stop
						...
						# Add a delay after file move to ensure operation completes
						Write-Host "Adding a delay after file move to ensure operation completes..."
						Start-Sleep -Seconds 3
					}
					else {
						Write-Host "Warning: Source file $sourcePath does not exist. Skipping."
					}
				}
				catch {
					Write-Host "Error during file operation: $_"
					...
					Write-Host "Falling back to copy operation..."
					Copy-Item "$($this.sdkPath)\XComGame\Script\$name.u" "$($this.stagingPath)\Script" -Force -WarningAction SilentlyContinue

					# Add a delay after file copy to ensure operation completes
					Write-Host "Adding a delay after file copy to ensure operation completes..."
					Start-Sleep -Seconds 3
				}
				finally {
					# Clean up backup if it exists and the move was successful
					if ((Test-Path $backupPath) -and (Test-Path $destPath)) {
						Remove-Item $backupPath -Force
					}
				}
			}
	...
	[void]_PrecompileShaders() {
		...
		if ($need_shader_precompile)
		{
			...
			$shaderCachePath = "$($this.stagingPath)/Content/$shaderCacheName"
			if (Test-Path $shaderCachePath) {
				Copy-Item -Path $shaderCachePath -Destination $this.buildCachePath
			} else {
				Write-Host "Shader cache file not found at $shaderCachePath - this is normal for mods without custom visual assets."
			}
		}
		else
		{
			Write-Host "No reason to precompile shaders, using existing"
			if (Test-Path $cachedShaderCachePath) {
				Copy-Item -Path $cachedShaderCachePath -Destination "$($this.stagingPath)/Content"
			} else {
				Write-Host "Cached shader file not found at $cachedShaderCachePath - this is normal for mods without custom visual assets."
			}
		}
	}
	...
	[void]_InvokeEditorCmdlet([StdoutReceiver] $receiver, [string] $makeFlags, [int] $sleepMsDuration) {
		...
		# Add a delay after the process exits to ensure file operations complete
		Write-Host "Adding a delay after process exit to ensure file operations complete..."
		Start-Sleep -Seconds 3
	}
